...
void LQRControllerNode::odomCallback(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  vehicleState_.vy = msg->twist.twist.linear.y; // get velocity
  ...
  vehicleState_.angular_velocity =  (vehicleState_.heading - vehicleState_.last_heading)/0.1;
  vehicleState_.last_heading = vehicleState_.heading;
...
void LQRControllerNode::controlTimerLoop(const ros::TimerEvent&) {
  ...
  if (!firstRecord_) {  //有定位数据开始控制
    ...
    // control_cmd.acceleration_pct = 0;
